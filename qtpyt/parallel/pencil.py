...
def _subarraytypes(comm, shape, axis, subshape, dtype):
    ...
    sizes = list(subshape)
    subdummy = datatype.Create_subarray(
        [1] * len(sizes), [1] * len(sizes), [0] * len(sizes)
    ).Commit()
    if any((s < 1 for s in sizes)):
        counts_displs = [[0] * p, [0] * p]
        datatypes = [subdummy] * p
        return tuple(counts_displs), tuple(datatypes)
    subsizes = sizes[:]
    substarts = [0] * len(sizes)
    counts_displs = [[1] * p, [0] * p]
    datatypes = []
    ...
    for i in range(min(N, p), p):
        counts_displs[0][i] = 0
        datatypes.append(subdummy)
    return tuple(counts_displs), tuple(datatypes)
...
class Transfer(object):
    ...
    def __init__(self, comm, shape, dtype, subshapeA, axisA, subshapeB, axisB):
        ...
        self._counts_displsA, self._subtypesA = _subarraytypes(
            comm, shape, axisA, subshapeA, dtype
        )
        self._counts_displsB, self._subtypesB = _subarraytypes(
            comm, shape, axisB, subshapeB, dtype
        )
        size = comm.Get_size()
    ...
    def forward(self, arrayA, arrayB):
        ...
        self.comm.Alltoallw(
            [arrayA, self._counts_displsA, self._subtypesA],
            [arrayB, self._counts_displsB, self._subtypesB],
        )
...
class Pencil(object):
    ...
    def transfer(self, pencil, dtype):
        ...
        return Transfer(
            comm, shape, dtype, penA.subshape, penA.axis, penB.subshape, penB.axis
        )
