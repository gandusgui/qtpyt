...
from .selfenergy import LeadSelfEnergy, BoxProbe
from .internalselfenergy import InternalSelfEnergy
from .greenfunction import GreenFunction
from .tools import subdiagonalize, cutcoupling, dagger, rotate_matrix, fermidistribution
from . import mpi
...
class TransportCalculator:
    ...
    def set(self, **kwargs):
        for key in kwargs:
            if key in [
                ...
                self.uptodate = False
                break
            elif key in ["eigenchannels", "dos", "pdos"]:
                self.uptodate = False
            elif key in ["energies"]:
                self.uptodate = False
                """ split energies among processors"""
                kwargs[key] = mpi.get_domain(kwargs[key])
    ...
    def initialize(self):
        ...
        if p["selfenergies"] is None:
            ...
            for eta, selfenergy in zip([p["eta1"], p["eta2"]], self.selfenergies):
        ...
        if p["greenfunction"] is None:
            ...
            self.greenfunction.set(align_bf=p["align_bf"], eta=p["eta"])
    ...
    def set_greenfunction(self, p):
        ...
        self.greenfunction = GreenFunction(
            selfenergies=self.selfenergies, H=h_mm, S=s_mm
        )
    ...
    #     self.initialize()
    #     self.greenfunction.take_bfs(bfs, apply)
    #     p = self.input_parameters
    #     if apply:
    #         self.uptodate = False
    #         p['h'] = self.greenfunction.H
    #         p['s'] = self.greenfunction.S
    #         for alpha, sigma in enumerate(self.selfenergies):
    #             p['hc{}'.format(alpha+1)] = sigma.h_im
    #             p['sc{}'.format(alpha+1)] = sigma.s_im
    #         return
    #     return
