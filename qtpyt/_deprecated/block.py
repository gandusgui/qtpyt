...
def block(rows, dtype=None, arrtype=None):
    if (
        (not _is_list_or_tup(rows))
        or len(rows) == 0
        or np.any([not _is_list_or_tup(row) for row in rows])
    ):
        raise RuntimeError(
            """
...
def block_diag(elems, dtype=None, arrtype=None):
    n = len(elems)
    return block(
        [[0] * i + [elem] + [0] * (n - 1 - i) for i, elem in enumerate(elems)],
        dtype=dtype,
        arrtype=arrtype,
    )
...
def _get_backend(rows, dtype, arrtype):
    ...
    elif arrtype is not None and re.search(
        "torch\..*(Variable|Parameter)", repr(arrtype)
    ):
...
class TorchBackend(Backend):
    ...
    def is_complete(self, x):
        return (
            re.search("torch\..*Tensor", str(x.__class__)) is not None
        ) and x.ndimension() == 2
...
class LinearOperatorBackend(Backend):
    ...
    def build(self, rows):
        ...
        row_sizes = [
            row[0].shape[0] if self.is_complete(row[0]) else row[0][0] for row in rows
        ]
        ...
        def matvec(v):
            ...
            for row, i, j in zip(rows, row_idxs[:-1], row_idxs[1:]):
                out[i:j] = sum(
                    lo.matvec(v[k:l])
                    for lo, k, l in zip(row, col_idxs[:-1], col_idxs[1:])
        ...
        def rmatvec(v):
            ...
            for col, i, j in zip(cols, col_idxs[:-1], col_idxs[1:]):
                out[i:j] = sum(
                    lo.rmatvec(v[k:l])
                    for lo, k, l in zip(col, row_idxs[:-1], row_idxs[1:])
                    if self.is_complete(lo)
                )
            return out
...
def test_toepliz():
    ...
    cols = [np.arange(12, 16).reshape(2, 2), np.arange(16, 20).reshape(2, 2)]
