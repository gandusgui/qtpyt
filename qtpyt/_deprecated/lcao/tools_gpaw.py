...
def get_bf_centers(atoms, basis=None):
    ...
    if calc is None or isinstance(calc, SinglePointCalculator):
        ...
        nao_a = [Basis(symbol, type).nao for symbol, type in zip(symbols, basis_a)]
    else:
...
def get_realspace_hs(
    ...
    if symmetry["time_reversal"]:
        ...
        weights_t_k = [1.0 / nkpts_t for k in range(nkpts_t)]
    ...
    for j, k_t in enumerate(ibzk_t_kc):
        for k_p in bzk_p_kc:
            ...
            c_k = np.exp(2.0j * np.pi * np.dot(k, R_c)) * weight_p_k
            h_skii[:, j] += c_k * h
            if s_kmm is not None:
                s_kii[j] += c_k * s
...
def remove_pbc(atoms, h, s=None, d=0, centers_ic=None, cutoff=None):
    if h.ndim > 2:
        raise KeyError("You have to run remove_pbc for each " "spin/kpoint separately.")
    L = atoms.cell[d, d]
    nao = len(h)
    dtype = h.dtype
...
def dump_hamiltonian(filename, atoms, direction=None, Ef=None):
    ...
    if atoms.calc.master:
        ...
        calc_data = {"weight_k": atoms.calc.weight_k, "ibzk_kc": atoms.calc.ibzk_kc}
...
def dump_hamiltonian_parallel(filename, atoms, direction=None, Ef=None):
    ...
    calc_data = {"k_q": {}, "skpt_qc": np.empty((nq, 3)), "weight_q": np.empty(nq)}
    ...
    for kpt in wfs.kpt_u:
        ...
        H_MM = wfs.eigensolver.calculate_hamiltonian_matrix(calc.hamiltonian, wfs, kpt)
...
def lead_kspace2realspace(
    h_skmm, s_kmm, bzk_kc, weight_k, direction="x", symmetry={"point_group": False}
):
...
def makeU(
    ...
    if world.rank == MASTER:
        ...
        Nw = len(wglobal_wG)
        print(
            "Estimated total (serial) mem usage: %0.3f GB"
            % (np.prod(gd.N_c) * Nw ** 2 * 8 / 1024.0 ** 3)
        )
    else:
        wglobal_wG = None
        Nw = 0
    ...
    if writeoptimizedpairs is not False:
        ...
        if world.rank == MASTER:
            ...
            P_aqp = dict([(a, np.dot(Uisq_qp, Px_pp)) for a, Px_pp in P_app.items()])
...
def makeV(
    ...
    for block, nq2 in enumerate(nq_r):
        ...
        for q1, q2 in np.ndindex(nq1, nq2):
            ...
            V_qq[q2 + q2start, q1] = coulomb.calculate(
                g1_qG[q1], g2_qG[q2], P1_ap, P2_ap
            )
